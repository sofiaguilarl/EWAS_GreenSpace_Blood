...
# Venn diagrams
venn_diagrams <- list(
   c('ALSPAC_M3_NDVI300_lf','GenR_M3_NDVI300_lf','HELIX_M3_NDVI300_lf')
)
...
plot_betas_boxplot(betas.data, paste(results_folder, 'BETAS_BoxPlot_EUR_M3_lf.png', sep="/"))
...
{
   # Precision_Plot(N)
   plot_precisionp(precplot.data, paste(results_folder,"precision_SE_N_EUR_M3_lf.png",sep='/'), main = "Precision Plot - 1/median(SE) vs sqrt(N)")

   # Precision_Plot(n)
   if(length(n) == length(N))
      plot_precisionp(precplot.data.n, paste(results_folder,  "precision_SE_N_EUR_M3_lf.png", sep='/'), main = "Subgroup Precision Plot -  1/median(SE) vs sqrt(n)")
